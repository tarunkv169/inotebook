...
export default function Note(props) {
    ...
    let updatenote=(currentnote)=>{   //8️⃣.4️⃣--- ⬆️-(8️⃣.4️⃣.1️⃣)---when edit icon click this func will run and setNote(with currentdata of editing note)      <----------------8️⃣.4️⃣-from Noteitem.js-------
                                                                     //🛑to do function on value of <input> ----like previous remain or erase--- then use (value={note.title etc})
    ...
    return (                                                        //5️⃣.0️⃣ Addnote comp ---------->goto Addnote.js5️⃣.1️⃣------>
                                                                    //8️⃣.0️⃣ creating modal   and   inserting(addnote form in className="modal-body") change id,name,className 🛑to do function on value of <input> ----like previous remain or erase--- then use (value={note.title etc})
        <>
        ...
        <div className="row my-5">
            ...
            {  
                notes.map((note)=>{                                 //6️⃣ using map() we use Noteitem.js because in this we integrate the fetch_notes
                    return <Noteitem key={note._id} updatenote={updatenote} showalert={props.showalert} note={note}/>   //7️⃣.0️⃣ now we make Noteitem.js in which we send (each note of notes) -----====> go to Noteitem.js with(🛑note={note}7️⃣.1️⃣---------->
